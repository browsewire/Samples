  ...
  public function realtorAction( Request $request)
	...
	public function updateRealtorAction(Request $request,$id)
	{
		...
		$file = $_FILES['file']['name'];
			...
			return $this->render('RARAdminBundle:Realtor:realtorUpdate.html.twig', array('realtor' => $realtor,'states' => $states));    
	}
...
return $this->redirect($this->generateUrl('RARAdminBundle_claim'));
   ...
   /*---- Start - Dashboard of Admin  -----*/ 
    ...
    ->getQuery()
    ->getResult();                    
		$dashboardDetails = array('totalRealtors' => $Realtors[0]['totalRealtors'],'totalClaims' => $Claims[0]['totalClaims'], 'totalProperties' => $Properties[0]['totalProperties'],'totalCategories' => $categories[0]['totalCategories'],'totalPlans' => $plans[0]  ['totalPlans']);			
