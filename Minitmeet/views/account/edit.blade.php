...
<div class="mobBoxin">
  ...
  <div class="create_timeTitle clearfix">
    <div class="stretch title_div"><input type="text" name="meetingtitle" value="{{ $minute->mTitle }}"></div>
    <div id="datetimepicker2" class="input-append date_div">
      <div id="create-input-small"><input name="meetingDate" class="datetimepicker" value="{{ $minute->selected_date }}" /></div>
    </div>
...
<!--     <div class="createsep mobile"></div>
    <div class="mobile at1 tag2">Click <span class="orange">"+"</span> Button to Add Person to Attendees</div> -->
...
<!--   <div class="createsep tab-desk"></div>
  <div class="clear"></div> -->
        ...
        <div class="clearfix">
          <div class="row-fluid">
            ...
            <div class="stretch agenda_div hidden">
              <textarea name="agenda" rows="10" cols="50" style="opacity:0;width:5px;">{{ $minute->mAgenda }}</textarea>
            </div>
          </div>
        </div><!--tab2_innerleft-->
      </div>
...
<!--         <div class="preview_section desktop">
          {{ HTML::image('img/thumb-minit.jpg', 'Minute Thumb') }}
          <div class="preview"><a class="button_preview" href="JavaScript:newPopup('<?php echo URL::to('/'); ?>/create_result?view=view&id=<?php echo $_SESSION['draft'];?>');">&nbsp;</a></div> 
        </div> -->
      </div>
...
<!-- <div class="grid_4 mob-right hidden">
  <form action ="{{ URL::to('/') }}/create_result" method="GET">
    <input type="hidden" name="id" value="{{ $_SESSION['draft'] }}">
    <input type="hidden" name="sendnote" value="sent">
  <button id="submitNote" type="submit" class="button_medium">SEND MINUTES</button>
  </form>
</div> -->
  <!-- <a href="JavaScript:enablecomment({{ $_SESSION['draft'] }});" id="enableComment"><span class="commentstate">Enable</span> Comment</a> -->
</div>
...
<!-- <div id="content">{{ Auth::user()->username }}{{ Auth::user()->email }}{{ Auth::user()->id }}</div>
 -->
...
(function(){
  ...
  /*Hiding bgsilver clearfix bgnsilver div*/
  ...
  jQuery( 'input[name="action"]' ).on( "click", function(){
    if(jQuery(this).is(":checked")){
      //var listSize = jQuery('#minus div div a').size();
...
/////
  jQuery('#minuteTab').click(function(){
    /*jQuery('.dynamicTitle').html('<strong>Create New Minutes</strong> <span class="indicator">(Minutes)</span>');*/
    jQuery('.create_header').removeClass('mob-hide').addClass('mob-show');
  });
  jQuery('#agendaTab').click(function(){
   /* jQuery('.dynamicTitle').html('<strong>Create New Meeting</strong> <span class="indicator">(Agenda)</span>');*/
    jQuery('.create_header').removeClass('mob-show').addClass('mob-hide');;
  });
  jQuery('#attendeeTab').click(function(){
   /* jQuery('.dynamicTitle').html('<strong>Create New Meeting</strong> <span class="indicator">(Attendees)</span>');*/
    jQuery('.create_header').removeClass('mob-show').addClass('mob-hide');;
  });
  ///////fetch_notes
  ...
  jQuery("#confirmNote").click(function(){
    ...
    jQuery("#submitNote").click(function(){
      ...
      sendnote:"sent"
        ...
        setTimeout(function() { 
			 window.location.href = "{{URL::to('account')}}";  
          console.log("AjaxStop Called 2");
          jQuery(".pwd_display").hide();
          jQuery("#confirmSendM").hide();         
